...
export async function signIn(_: AuthFormState, formData: FormData): Promise<AuthFormState> {
  ...
  if (!credentials) {
    return { message: 'Enter a valid email address and password.', status: 'error' };
  }
  ...
  if (error) {
    return { message: 'Unable to sign in with those credentials.', status: 'error' };
  }
...
export async function signUp(_: AuthFormState, formData: FormData): Promise<AuthFormState> {
  ...
  await supabase.auth.signUp({
    ...credentials,
    ...(origin
      ? { options: { emailRedirectTo: new URL('/auth/callback?next=/dashboard', origin).toString() } }
      : {}),
  });
